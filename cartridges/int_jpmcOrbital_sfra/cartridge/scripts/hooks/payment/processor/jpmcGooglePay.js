...
function Handle(basket, paymentInformation) {
    ...
    var securityCode = paymentInformation.jpmcOGooglePayForm.jpmcOGooglePayForm.securityCodeGoogle.value;
    var saveCustomerPaymentCheckboxGP = paymentInformation.jpmcOGooglePayForm.jpmcOGooglePayForm.saveCustomerPaymentCheckboxGP.checked;
    if ((empty(securityCode) || securityCode.search(/^[0-9]{3,4}$/) === -1)) {
        var securityCodeErrors = {};
        securityCodeErrors.dwfrm_billing_jpmcOGooglePayForm_securityCodeGoogle = Resource.msg('error.invalid.securityCode.number', 'creditCard', null);
        JPMCLogger.error('jpmcGooglePay.js (Handle): securityCodeGoogle errors: {0}.', securityCodeErrors);

        return { fieldErrors: [securityCodeErrors], error: true };
    }
    var paymentMethodId = JPMCOrbitalConstants.JPMC_ORBITAL_GOOGLEPAY_METHOD;
    transactionHelper.getHandleTransaction(null, currentBasket, paymentMethodId, securityCode, saveCustomerPaymentCheckboxGP, paymentInformation.googlepay.value);
