...
function Handle(basket, paymentInformation) {
    ...
    var securityCode = paymentInformation.jpmcOVisaPayForm.jpmcOVisaPayForm.securityCodeVisa.value;
    var saveCustomerPaymentCheckboxVP = paymentInformation.jpmcOVisaPayForm.jpmcOVisaPayForm.saveCustomerPaymentCheckboxVP.checked;
    if ((empty(securityCode) || securityCode.search(/^[0-9]{3,4}$/) === -1)) {
        var securityCodeErrors = {};
        securityCodeErrors.dwfrm_billing_jpmcOVisaPayForm_securityCodeVisa = Resource.msg('error.invalid.securityCode.number', 'creditCard', null);
        JPMCLogger.error('jpmcOrbitalVisaPay.js (Handle): securityCodeVisa errors: {0}.', securityCodeErrors);

        return { fieldErrors: [securityCodeErrors], error: true };
    }
    var paymentMethodId = JPMCOrbitalConstants.JPMC_ORBITAL_VISA_CHECKOUT_METHOD;
    transactionHelper.getHandleTransaction(null, currentBasket, paymentMethodId, securityCode, saveCustomerPaymentCheckboxVP, null, paymentInformation.visaPay.value);
